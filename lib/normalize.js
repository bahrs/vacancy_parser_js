function cleanText(s) {
  ...
  function inferLevel(role, text) {
    ...
    // "1–3 года", "от 2-х и более лет" → junior+
    if (/\b(1\s*[–-]\s*3|от\s*2[-\s]х?\s*и?\s*более|от\s*1\s*до\s*3|1\s*-\s*3)\s*(год|лет|года)/i.test(t)) return "junior+";
    // "3–5 лет", "от 3-х лет" → middle
    if (/\b(3\s*[–-]\s*5|от\s*3[-\s]х?\s*и?\s*более|от\s*3\s*до\s*5|3\s*-\s*5)\s*(год|лет|года)/i.test(t)) return "middle";
    // "5+ лет", "от 5 лет" → senior
